...
% John's changes: load blockB_gb, use audioread/write, only look at first
% receiver, narr indexing
...
fileroot = 'wall_mount_ditch_wall'
ARRFIL   = [ fileroot ]%'.arr' ];
pcmfile  = [ fileroot '.pcm' ];

% source timeseries
%STSFIL = 'Ricker.wav'
STSFIL = 'srcsignal1500.wav'
...
ir  = 1 %length( Pos.r.range );
isd = 1 %length( Pos.s.depth );

deltat = 1 / sample_rate;
nt     = round( T / deltat );	% number of time points in rts
rtsmat = zeros( nt, length( Pos.r.range ) );

for ird = 1 : 1; %length( Pos.r.depth );
    for ir = 1 : 1; %length( Pos.r.range )   % loop over receiver ranges
        disp( [ ird, ir ] )
        ...
        for iarr = 1 : Arr.Narr( ir, ird, isd )   % loop over arrivals
            ...
            Tarr = Arr.delay( ir, ird, iarr, isd ) - tstart;   % arrival time relative to tstart
