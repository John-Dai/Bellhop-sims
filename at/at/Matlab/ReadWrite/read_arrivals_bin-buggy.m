...
fid = fopen( [ ARRFile, '.arr.mat'], 'r');	% open the file

% read the header info

fseek( fid, 4, -1 );
load blockB_gb.arr.mat

freq = fread( fid, 1, 'float' );

Nsd  = Pos.Nsd;%fread( fid, 1, 'long'  );
Nrd  = Pos.Nrd;%fread( fid, 1, 'long'  );
Nrr  = Pos.Nrr;%fread( fid, 1, 'long'  );
...
%          Narr = fread( fid, 1, 'int', 8 );
         ...
         if Narr > 0   % do we have any arrivals?
            ...
            Narr1 = min( Narr, Narrmx ); % we'll keep no more than Narrmx values
