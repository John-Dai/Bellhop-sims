...
if ( NewRay )
   if ( Nt >= MxNarr )   % space available to add an arrival?
      ...
      if ( amp > Arr.A( id, ir, iArr_rand ) )
         Arr.A(     id, ir, iArr_rand ) = amp;  	 % amplitude
         Arr.delay( id, ir, iArr_rand ) = delay;	 % delay time
         Arr.angle( id, ir, iArr_rand ) = angle;	 % angle
      end
      
   else     % space available; add another arrival to the table
      Arr.Narr(  id, ir         ) = Nt + 1;   % # of arrivals
      for abc = 1:length(Nt)
      Arr.A(     id, ir, Nt(abc) + 1 ) = amp;		 % amplitude
      Arr.delay( id, ir, Nt(abc) + 1 ) = delay;	 % delay time
      Arr.angle( id, ir, Nt(abc) + 1 ) = angle;	 % angle
      end
   end
